...
-export([send/1, recv/1, recv/2, ws_recv/0, recv_body/0, recv_body/1, stream_body/3]).
-export([start_response/1, start_response_length/1, start_raw_response/1, start_websocket/1]).
...
ws_recv() ->
    case gen_tcp:recv(Socket, 1, 10) of
	{ok, Data} ->
	    put(?SAVE_RECV, true),
	    Data;
	{error, timeout} ->
...
start_websocket({Code, ResponseHeaders}) ->
    F = fun ({K, V}, Acc) ->
                [make_io(K), <<": ">>, V, <<"\r\n">> | Acc]
        end,
    End = lists:foldl(F, [<<"\r\n">>],
			...
			lists:append(
				websocket_headers(),
				ResponseHeaders))),
    send([make_version(Version), make_websocket_code(Code), <<"\r\n">> | End]).
...
make_websocket_code(X) when is_integer(X) ->
    [integer_to_list(X), " Web Socket Protocol Handshake"];
make_websocket_code(Io) when is_list(Io);is_binary(Io) ->
    Io.
